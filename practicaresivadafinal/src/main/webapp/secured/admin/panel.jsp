...
<body>
	...
	<h2>
		<a
			href="${pageContext.request.contextPath}/secured/admin/usermanagement.jsp">User
			Management</a>
	</h2>

	<h2>Lista espacios</h2>
	<ul>
	<%
		HashSet<Espacio> listaEspacios = (HashSet<Espacio>) application.getAttribute("LISTA_ESPACIOS");
	...
	%>
		<li>
			...
			<form action="<%= request.getContextPath() %>/control" method="post" style="display:inline;">
				<input type="hidden" name="accion" value="eliminarespacio">
				<input type="hidden" name="<%= Constantes.PARAM_NOMBRE %>" value="<%= espacio.getNombre() %>">
	...
	<form action="<%= request.getContextPath() %>/control" method="POST">
		...
		<input type="datetime-local" name="<%=Constantes.PARAM_FECHA%>" required>
